...
class TestKVCacheStoreLayerSendingThread(unittest.TestCase):
    ...
    def test_multi_group_tasks_commit_and_revoke_at_independent_boundaries(self):
        ...
        thread.request_queue.put(physical_zero_tasks)
        thread._handle_request(physical_zero_tasks)

        physical_one_tasks = [make_task(1, 0, 1)]
        thread.add_stored_request(request.req_id)
        thread.request_queue.put(physical_one_tasks)
        thread._handle_request(physical_one_tasks)
