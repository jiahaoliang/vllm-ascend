...
class TestMooncakeBackendMethods(unittest.TestCase):
    ...
    def test_whole_key_debug_logs_legacy_paths_only(self):
        ...
        self.assertFalse(
            any(call.args[:2] == ("%s %s", "[KVPOOL_RANGE_DEBUG]") for call in ranged_log_info.call_args_list)
        )
    ...
    def test_layerwise_methods_delegate(self):
        ...
        b.store.batch_put_from_multi_buffer_ranges.assert_called_once_with(["k"], [[100]], [[64]], [[0]])
        b.store.batch_get_into_multi_buffer_ranges.assert_called_once_with(["k"], [[200]], [[64]], [[0]])
    ...
    def test_validate_layerwise_support_reports_all_missing_methods(self):
        ...
        self.assertTrue(hasattr(b, "validate_layerwise_support"))
        with self.assertRaisesRegex(RuntimeError, "batch_put_session_start.*batch_get_session_end"):
