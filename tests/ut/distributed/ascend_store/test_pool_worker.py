...
class TestKVPoolWorkerMooncakeLayerSessions(unittest.TestCase):
    ...
    def test_multi_group_store_mask_maps_logical_key_to_clipped_block_table(self):
        ...
        worker._process_save_for_layer_batch([request], 0, group_id=1)
    ...
    def test_multi_group_load_mask_maps_logical_key_to_clipped_block_table(self):
        ...
        worker._prepare_mooncake_layerwise_sessions([request])
        worker.layer_load_tasks = [[]]
        worker._process_load_for_layer_batch([request], 0, group_id=1)
    ...
    def test_multi_group_put_start_uses_group_keys_sizes_and_aligned_results(self):
        ...
        self.assertTrue(
            all(key.startswith("model@0@") for key in group_zero_call.args[0])
        )
        self.assertTrue(
            all(key.startswith("model@1@") for key in group_one_call.args[0])
        )
        self.assertEqual(
    ...
    def test_align_state_group_uses_per_rank_key_and_save_owner(self):
        ...
        worker._prepare_mooncake_layerwise_sessions([request])
        worker.layer_save_tasks = [[]]
        worker._process_save_for_layer_batch(
            [request],
            layer_id=0,
            group_id=1,
        )
