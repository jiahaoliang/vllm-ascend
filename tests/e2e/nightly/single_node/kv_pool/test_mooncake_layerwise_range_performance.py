...
_CONFIG_ENV = "VLLM_ASCEND_NIGHTLY_MOONCAKE_CONFIG"
_MIN_GBPS_ENV = "VLLM_ASCEND_NIGHTLY_KVPOOL_MIN_GBPS"
_MAX_P95_MS_ENV = "VLLM_ASCEND_NIGHTLY_KVPOOL_MAX_P95_MS"
    ...
    for _ in range(iterations):
        for layer_index in range(shape.layer_count):
            ...
            for row_indices in row_batches:
                ...
                expected_results = [shape.row_bytes] * len(batch_keys)
...
def test_mooncake_layerwise_ranged_transfer_performance(
    ...
    if not config_value:
        pytest.skip(f"set {_CONFIG_ENV} to run the Mooncake/NPU performance gate")

    config_path = Path(config_value)
    if not config_path.is_file():
        pytest.fail(f"{_CONFIG_ENV} does not name a file: {config_path}")
    ...
    min_gbps = _required_positive_float(_MIN_GBPS_ENV)
    max_p95_ms = _required_positive_float(_MAX_P95_MS_ENV)
