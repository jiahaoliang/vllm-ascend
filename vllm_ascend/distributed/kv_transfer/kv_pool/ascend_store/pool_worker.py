...
class KVPoolWorker:
        ...
        for group_id in range(getattr(self, "num_kv_cache_groups", 1)):
            ...
            if group_num_layers is None or group_block_len is None:
                if getattr(self, "num_kv_cache_groups", 1) > 1:
                    raise ValueError(
                        f"missing registered layerwise metadata for group {group_id}"
                    )
            ...
            if layer_entry_indices is None:
                caches_per_layer, remainder = divmod(
                    len(group_block_len), group_num_layers
                )
                if remainder or caches_per_layer <= 0:
                    raise ValueError(
                        f"group {group_id} cache entries are not covered by layer tuples"
                    )
                ...
                group_page_size = sum(
                    group_block_len[index]
                    for index in layer_entry_indices[0]
                )
            builders.append(
                LayerBatchBuilder(
                    self.token_database,
        ...
        if self.backend_name == "mooncake":
            ...
            for group_id, builder in enumerate(builders):
                ...
                if not physical_to_local and len(builders) == 1:
                    physical_to_local = tuple(
                        (layer_id, layer_id)
                        for layer_id in range(builder.num_layers)
                    )
                ...
                if (
                    ...
                    or any(
                        physical_layer < 0 or physical_layer >= self.num_layers
                        for physical_layer in physical_layers
                    )
                ):
                    raise ValueError(
                        f"group {group_id} physical/local layer mapping is incomplete or ambiguous"
                    )
                effective_block_size = self._get_effective_group_block_size(
                    group_id
                )
                if effective_block_size <= 0 or builder.object_size <= 0:
                    raise ValueError(
                        f"group {group_id} block and object sizes must be positive"
                    )
                plans.append(
    ...
    def _group_local_block_index(
        ...
        allocated_logical_blocks = (
            request.target_token_len + effective_block_size - 1
    ...
    def _start_kv_transfer_threads(self) -> None:
        ...
        if self.use_layerwise:
            ...
            if self.use_block_key_layerwise and is_kv_save_role(
                self.kv_role,
            ...
            elif is_kv_save_role(self.kv_role, self.consumer_is_to_put):
                ready_event_sending = threading.Event()
            ...
            if is_kv_save_role(self.kv_role, self.consumer_is_to_put):
                ready_event_sending = threading.Event()
    ...
    def _get_effective_group_block_size(self, group_id: int) -> int:
        cache_family = self._get_group_family(getattr(self, "kv_cache_group_families", ["default"]), group_id)
    ...
    def _infer_cache_group_metadata(self, group_id: int, layer_names: list[str]):
        ...
        layer_entry_indices: dict[int, list[int]] = {}
        physical_layer_order: list[int] = []
        for layer_name in layer_names:
            phys = self._extract_physical_layer_index(layer_name)
            if phys >= self.num_layers:
                continue
            if phys not in layer_entry_indices:
                layer_entry_indices[phys] = []
                physical_layer_order.append(phys)
            ...
            for cache in self._as_cache_tuple(cache_or_caches):
                ...
                layer_entry_indices[phys].append(len(group_block_lens) - 1)
        ...
        self.group_num_layers[group_id] = len(physical_layer_order)
        self.group_layer_entry_indices[group_id] = tuple(
            tuple(layer_entry_indices[physical_layer])
            for physical_layer in physical_layer_order
        )
    ...
    def _process_save_for_layer_batch(
        ...
        # PutStart and layer tasks must have the same owner. Other ranks in a
        # put_step group share the MLA latent cache and only open read sessions.
        ...
        for request in requests:
            if request.can_save is None or not request.can_save:
                continue
            if self.use_block_key_layerwise and self.backend_name == "mooncake":
                group_keys = request.save_keys_by_group.get(group_id)
                if group_keys is None:
                    continue
                populated_slots = [
                    group_keys.block_offset + index
                    for index, key in enumerate(group_keys.block_keys)
                    if key is not None
                ]
                partial_block_index = group_keys.last_block_index if group_keys.last_block_key is not None else None
                if not populated_slots and partial_block_index is None:
                    continue
                request_block_ranges.append(
                    LayerBlockRange(
                        request=request,
                        start_block=(min(populated_slots) if populated_slots else 0),
                        end_block=(max(populated_slots) + 1 if populated_slots else 0),
            ...
            request_group_block_ids = self._request_group_block_ids(
                request,
                group_id,
            )
            save_end_block = min(save_end_block, len(request_group_block_ids))
            ...
            if getattr(self, "use_memcache_gva_layerwise", False) and save_start_block < save_end_block:
            ...
            group_keys = request.save_keys_by_group.get(group_id)
            partial_block_index = (
                group_keys.last_block_index
                if (
                    self.use_block_key_layerwise
                    and self.backend_name == "mooncake"
                    and group_keys is not None
                    and group_keys.last_block_key is not None
                )
                else request.partial_block_index
            )
    ...
    def _process_load_for_layer_batch(
        ...
        for request in requests:
            if self.use_block_key_layerwise and self.backend_name == "mooncake":
                group_keys = request.load_keys_by_group.get(group_id)
                if group_keys is None:
                    continue
                populated_slots = [
                    group_keys.block_offset + index
                ...
                partial_block_index = (
                    group_keys.last_block_index
                    if group_keys.last_block_key is not None
                    else None
                )
                if not populated_slots and partial_block_index is None:
                    continue
            ...
            if (request.last_block_gva is not None or request.load_last_block_key is not None) and (
    ...
    def _end_mooncake_load_keys(self, keys: list[str]) -> None:
        ...
        try:
            ...
            if result != 0:
                logger.error(
                    "Mooncake batch_get_end failed for keys=%s result=%s",
                    keys,
                    result,
                )
        except Exception as exc:
            logger.error("Mooncake batch_get_end raised for keys=%s: %s", keys, exc)

    def _release_mooncake_requests_for_retry(self, req_ids: set[str]) -> None:
        with self._load_session_lock:
            keys_to_end = self._mooncake_session_tracker.release_for_retry(req_ids)
            self._end_mooncake_load_keys(keys_to_end)

    def _release_mooncake_requests_terminal(self, req_ids: set[str]) -> None:
        with self._load_session_lock:
            keys_to_end = self._mooncake_session_tracker.release_terminal(req_ids)
            self._end_mooncake_load_keys(keys_to_end)

    def _release_failed_mooncake_get_attempts(
        self,
    ...
    def _prepare_mooncake_put_session(self, request: ReqMeta) -> None:
        ...
        for group_id in range(getattr(self, "num_kv_cache_groups", 1)):
            ...
            for logical_block_index in range(start_block, end_block):
                ...
                full_key_slots.append(
                    (key, local_block_index, logical_block_index)
                )

            key_slots: list[tuple[str, int | None, int]] = []
            if full_key_slots:
                group_keys.block_offset = min(
                    local_block_index
                    for _, local_block_index, _ in full_key_slots
                )
                last_local_block = max(
                    local_block_index
                    for _, local_block_index, _ in full_key_slots
                )
                group_keys.block_keys = [None] * (
                    last_local_block - group_keys.block_offset + 1
                )
            ...
            with self._put_started_keys_lock:
                ...
                new_keys = [
                    ...
                    if key not in self._put_started_keys
                    and key not in self._put_pending_revoke_keys
                ]
            ...
            if new_keys:
                try:
                    ...
                    newly_started = {
                        key
                        for key, result in zip(new_keys, results, strict=True)
                        if result == 0
                    }
                    with self._put_started_keys_lock:
    ...
    def _prepare_mooncake_get_session(
        ...
        for key, group_id, logical_block_index in load_entries:
            ...
            if len(group_keys.block_keys) <= block_index:
                group_keys.block_keys.extend(
                    [None] * (block_index + 1 - len(group_keys.block_keys))
                )
    ...
    def process_layer_data(self, requests: list[ReqMeta]) -> None:
        ...
        for physical_layer in range(self.num_layers):
            ...
            if group_layers is None:
                if self.num_kv_cache_groups > 1:
                    raise ValueError(
                        f"missing layerwise group mapping for physical layer {physical_layer}"
                    )
    ...
    def wait_for_layer_load(self) -> None:
        ...
        if not should_wait:
            ...
            if self._layer_load_aborted.is_set() or self.current_layer == self.num_layers - 1:
                self._finish_current_mooncake_load_sessions()
            if not is_kv_save_role(self.kv_role, self.consumer_is_to_put):
                # Pure consumers have no save hook to advance the shared
                # attention-layer cursor after this load step.
        ...
        if not is_finish:
            ...
            # batch_get_end must not race an in-flight ranged read. The
            # exception-safe Receiver always sets this event after its backend
            # call returns, including error paths.
            if self.backend_name == "mooncake":
                is_drained = self.layer_load_finished_events[self.current_layer].wait(timeout=10)
                if not is_drained:
                    raise TimeoutError(
                ...
                # Preserve the inherited memcache drain barrier exactly.
                self.layer_load_finished_events[self.current_layer].wait()
            is_finish = True
        logger.debug(">>>>>>>>>>>>>>>>>>>> clear load layer %d", self.current_layer)
        self.layer_load_finished_events[self.current_layer].clear()
        if is_finish and (self._layer_load_aborted.is_set() or self.current_layer == self.num_layers - 1):
            self._finish_current_mooncake_load_sessions()
        if not is_kv_save_role(self.kv_role, self.consumer_is_to_put):
            self.current_layer += 1
