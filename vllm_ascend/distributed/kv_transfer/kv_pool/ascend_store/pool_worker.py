...
class KVPoolWorker:
            ...
            for request in requests:
                ...
                if can_load:
                    ...
                    if use_key_major_ranges and request.load_last_block_key is not None and (
                        ...
                        partial_block_index = None
                    if load_start_block < full_blocks or partial_block_index is not None:
    ...
    def _build_shared_save_data(self) -> None:
        ...
        if not isinstance(self.kv_send_thread, KVCacheStoreLayerSendingThread):
            return
        for group_id in range(self.num_kv_cache_groups):
            first_task = next(
                (
    ...
    def process_layer_data(self, requests: list[ReqMeta]) -> None:
                ...
                if save_ranges:
                    self.layer_save_tasks[physical_layer].append(
                        LayerTransferTask(
                            ...
                            use_key_major_ranges=use_key_major_ranges,
                        )
                    )
        ...
        if self.use_memcache_gva_layerwise and any(self.layer_load_tasks):
    ...
    def wait_for_layer_load(self) -> None:
        ...
        if not should_wait:
            ...
            if self.backend_name == "mooncake" and (
    ...
    def get_finished(self, finished_req_ids: set[str], meta: AscendConnectorMetadata) -> tuple[set[str], set[str]]:
        if finished_req_ids and self.use_memcache_gva_layerwise:
