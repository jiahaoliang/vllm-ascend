...
* **Block-key topology**: Mooncake and memcache support TP-only topology. PP,
  ...
  behavior.
* **Hybrid KV cache**: Memcache layerwise raises `NotImplementedError` when
  layer reuse is enabled for multiple KV cache groups; its existing transfer
  path without buffer reuse retains multi-group support. Mooncake multi-group
  layerwise transfer is deferred and is not supported by this implementation.
