...
* **Block-key topology**: Mooncake and memcache support TP-only topology. PP,
  ...
  behavior.
* **Hybrid KV cache**: Layerwise raises `NotImplementedError` only when layer
  reuse is enabled for multiple KV cache groups. Layerwise transfer without
  buffer reuse retains multi-group support.
